...
BASE_PATH=~/workspace/data/2control_exp/sec_Min/maps/

#output path of histories, model
OUT_PATH=~/workspace/data/2control_exp/1run_seconds/results/
...
do
    ...
    python3 ../NN_WORKFLOWS/classify_aggregation_maps.py "$BASE_PATH${MAP_FOLDERS[$i]}" $RESOLUTION $EPOCHS $BATCH_SIZE $VALIDATION_SPLIT "$OUT_PATH${MAP_FOLDERS[$i]}/" "$NAME${MAP_FOLDERS[$i]}"
